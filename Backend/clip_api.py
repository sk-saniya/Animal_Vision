...
# # ──────────────────────────────────────────────────────────────
# app = Flask(__name__)
# CORS(app)   # allow all origins — so any frontend can connect

# # ──────────────────────────────────────────────────────────────
# # LOAD CLIP (once at startup)
# # ──────────────────────────────────────────────────────────────
# print("[clip_api] Loading CLIP model …")
...
# # ──────────────────────────────────────────────────────────────
# # LOAD animals_data.txt
# # ──────────────────────────────────────────────────────────────
# def load_animals_txt(path: Path) -> List[str]:
#     names = []
#     if not path.exists():
#         print(f"[clip_api] WARNING: {path} not found.")
#         return names
...
# # ──────────────────────────────────────────────────────────────
# # BUILD / LOAD TEXT EMBEDDINGS CACHE
# # ──────────────────────────────────────────────────────────────
...
#         print(f"[clip_api] Loading cached embeddings from {cache_path} …")
...
#     print(f"[clip_api] Cache saved → {cache_path} ✅")
...
# # ──────────────────────────────────────────────────────────────
# # CLIP ZERO-SHOT PREDICT → ONE name + accuracy
# # ──────────────────────────────────────────────────────────────
...
# def health():
#     """
#     Frontend can call this to check if the server is running.

#     Response:
#         { "status": "ok", "animals_loaded": 25000 }
#     """
#     return jsonify({
#         "status"         : "ok",
...
#     return jsonify({
#         "status": "ok",
#         "message": "Animal Vision backend is running",
#         "health": "/health",
#         "predict": "/predict"
#     })
...
#         except ValueError:
#             top_k = 5
#         top_k = max(1, min(top_k, len(animal_names)))
#         top_probs, top_idxs = torch.topk(probs, top_k)
#         predictions = [{
#             "label": animal_names[idx],
...
#         accuracy = round(probs[best_idx].item() * 100.0, 2)
#     except Exception as e:
#         return jsonify({"error": f"Prediction failed: {str(e)}"}), 500

#     # Return predictions (always include top prediction)
#     return jsonify({
...
#     print("\n[clip_api] Server starting …")
#     print("[clip_api] Endpoints:")
...
clip_api.py
-----------
Flask REST API backend with NON-ANIMAL GUARD.

If the uploaded image is NOT an animal (e.g. human, car, bag, tree,
phone, table, computer, etc.), the API returns a 400 error with a
clear message instead of making a wrong prediction.

ENDPOINTS:
    POST /predict   → image file → animal name + accuracy
                      or 400 error if not an animal image
