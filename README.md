...
The 100‑animal image dataset can be downloaded from:

```bash
# Download zip archive
https://www.scidb.cn/en/detail?dataSetId=e2ebd46cb1304a82bab54a8873cb3004
# Extract contents into the backend folder
unzip animal_dataset.zip -d Backend/Animal
```

Alternatively, you can clone the dataset repository:

```bash
git clone https://github.com/example/animal-dataset.git Backend/Animal
```

Make sure the images are placed under `Backend/Animal/<animal_name>/` as shown in the repository.
...
│   ├─ predict.py
│   ├─ animals_data.txt
│   └─ Animal/            # 100‑animal image dataset
│       ├─ lion/
│       ├─ tiger/
│       └─ ...
├─ Frontend/
